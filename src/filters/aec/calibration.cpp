...
uint8_t* AECCalibration::generateCalibrationWav(
    ...
    size_t* outSize)
{
    ...
    // White noise section
    generateWhiteNoise(sGeneratedSignal.data(), whiteNoiseSamples, SIGNAL_AMPLITUDE);

    // Sine sweep section
    generateSineSweep(
        sGeneratedSignal.data() + whiteNoiseSamples,
        sineSweepSamples,
        sampleRate,
...
void AECCalibration::captureSignals(
    ...
    size_t micLen)
{
    ...
    // This captures the real transfer function including speaker frequency response,
    // DAC characteristics, and any distortion - critical for accurate deconvolution
    sRefCapture.assign(referenceBuffer, referenceBuffer + referenceLen);
    sMicCapture.assign(micBuffer, micBuffer + micLen);

    printf("[AEC Calibration] Captured signals: ref=%zu samples, mic=%zu samples\n",
           referenceLen, micLen);
}
...
CalibrationResult AECCalibration::analyze(unsigned int sampleRate)
{
    ...
    printf("[AEC Calibration] analyze: ref samples=%zu (energy=%.4f) mic samples=%zu (energy=%.4f)\n",
    ...
    int optimalDelay = findOptimalDelay(
        sRefCapture.data(),
        sRefCapture.size(),
        sMicCapture.data(),
        sMicCapture.size(),
    ...
    float echoGain = estimateEchoGain(
        sRefCapture.data(),
        sMicCapture.data(),
        std::min(sRefCapture.size(), sMicCapture.size()),
        optimalDelay);

    // Compute impulse response via FFT deconvolution
    // Align signals first using the optimal delay
    size_t alignedLen = std::min(sRefCapture.size(), sMicCapture.size() - optimalDelay);
    if (alignedLen > 1000) {  // Need enough samples for good deconvolution
        result.impulseResponse = computeImpulseResponse(
            ...
            alignedLen,
            2048);  // Match NLMS filter length (43ms @ 48kHz - mobile-optimized)
    } else {
        printf("[AEC Calibration] Warning: Not enough aligned samples for impulse response (%zu)\n", alignedLen);
    }
    ...
    printf("[AEC Calibration] Result: delay=%dms gain=%.3f corr=%.3f impulseLen=%zu\n",
           result.delayMs, result.echoGain, result.correlation, result.impulseResponse.size());
...
void AECCalibration::generateWhiteNoise(
    float* buffer,
    size_t samples,
    float amplitude)
{
...
void AECCalibration::generateSineSweep(
    float* buffer,
    size_t samples,
    unsigned int sampleRate,
    float startFreq,
...
int AECCalibration::findOptimalDelay(
    ...
    float* outCorrelation)
{
    ...
    for (int delay = 0; delay < maxDelaySamples && delay < (int)recLen - (int)windowLen; ++delay) {
        float corr = 0.0f;
...
float AECCalibration::estimateEchoGain(
    const float* reference,
    const float* recorded,
    size_t len,
    int delay)
...
void AECCalibration::writeWavHeader(
    uint8_t* buffer,
    unsigned int sampleRate,
    unsigned int channels,
    size_t numSamples)
...
std::vector<float> AECCalibration::computeImpulseResponse(
    const float* reference,
    const float* mic,
    size_t len,
    int filterLength)
{
    ...
    // Zero-pad to power of 2 (at least 2x signal length for linear convolution)
    size_t fftSize = nextPowerOf2(len * 2);

    // SoLoud FFT uses interleaved complex format: [real0, imag0, real1, imag1, ...]
    ...
    // Apply Hann window to prevent spectral leakage, then copy to FFT buffers
    ...
    // Complex division: H = Mic / Ref (with regularization to avoid division by zero)
    // For complex numbers: (a+bi) / (c+di) = (ac+bd)/(c²+d²) + (bc-ad)/(c²+d²)i
    ...
    for (size_t i = 0; i < fftSize; ++i) {
        ...
        if (power > maxRefPower) maxRefPower = power;
    }
    // Use 1% of peak power as regularization - prevents instability while preserving detail
    float regularization = 0.01f * maxRefPower + 1e-10f;
    printf("[AEC Calibration] FFT deconvolution: fftSize=%zu maxRefPower=%.6f reg=%.6f\n",
    ...
    printf("[AEC Calibration] Computed impulse response: len=%d energy=%.4f peak=%.4f\n",
